...
impl GameState for State {
    fn tick(&mut self, ctx : &mut Rltk) {
        ...
        match newrunstate {
            ...
            RunState::MainMenu { .. } => {
                let result = gui::main_menu(&self.ecs, ctx);
                match result {
...
fn main() -> rltk::BError {
    ...
    gs.ecs.register::<SimpleMarker<SerializeThis>>();
    gs.ecs.register::<SerializationHelper>();

    gs.ecs.insert(SimpleMarkerAllocator::<SerializeThis>::new());
